...
@Deprecated public class OldIntake implements Intake {
    ...
    public OldIntake(DcMotor arm, DcMotor extender, Roller[] rollers) {
        this.arm = arm;
    ...
    @NonNull
    public static OldIntake standard(HardwareMap hardwareMap) {
        Roller roller = new Roller(hardwareMap.get(Servo.class, rollerMotorName));
        Roller[] servos = {roller, roller};
        return new OldIntake(hardwareMap.get(DcMotor.class, armMotorName), hardwareMap.get(DcMotor.class, extenderMotorName), servos);
    }
}
